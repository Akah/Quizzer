var count = 0, //each iteration, 0 being question one. increased after each question answered
    given, //the value set to the chosen answer
...
function next() { //click of next button do this:
	...
	if (answer[count] === undefined) { //if count loop finished then do:
		...
		document.getElementById("result").innerHTML = "wrong answers: <br>" + wrong + "<br>" + ((correct.length / question.length) * 100) + "%"; //gives percentage score
...
function resultColour(x) {
	if (valid === true) {
		...
		$('#ans' + x).css('background-color', '#4CAF50');//chosen answer
	} else {
		$('.answer').css('background-color', '#95a5a6');
		$('#ans' + x).css('background-color', '#f44336');//chosen answer
	}
}
